...
# Download and setup the Inception model
inception_url = "https://tfhub.dev/google/tf2-preview/inception_v3/feature_vector/4"
inception_model = tf.keras.Sequential([
    hub.KerasLayer(inception_url, input_shape=(img_size, img_size, 3))
])
...
# Get the style and content features from the Inception model
style1_features = inception_model(style1_img)
style2_features = inception_model(style2_img)
content_features = inception_model(content_img)
...
generated_img = inception_model(mixed_features, training=False)
